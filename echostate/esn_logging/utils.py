    ...
    try:
        ...
        if tc.numel() > 0:
            ...
            if finite.any():
                ...
                info[f"sat_frac_|x|>{sat_thresh:.2f}"] = float((tc.abs() > sat_thresh).float().mean().item())
        k = min(6, tc.numel())
        if k > 0:
...
def log_tensor(logger: logging.Logger, t: torch.Tensor, name: str,
               level: int = logging.DEBUG, include_values: bool = False) -> None:
