...
import numpy as np
import torch
from echostate import mean_absolute_error 
...
class ESNPredictor:
    ...
    def predict_sequence(self, esn, z_test):
        ...
        """
        X_test = torch.tensor(z_test[:-1], dtype=torch.float32, device=self.device).unsqueeze(-1)  # (T,1)
        preds_t = esn.infer(X_test)            # (T - washout, 1)
        preds = preds_t.detach().cpu().numpy().flatten()
...
def Heisen_tune(predictor, study_name, study_loc, washout, seed, n_trials, param_name, dt=None, plots=False, learning_algo="inv"):
    ...
    space = DefaultSpace(
        ...
        sparsity=(0.01, 0.1),
        leak_rate=(0.2, 1.0),
        input_scaling=(0.1, 0.3),
        bias_scaling=(0.0, 0.0),
        ridge_param=(1e-11, 1e-3),
        feedback=(0, 0),
    )

    study_dir = study_loc
    study, best_cfg = run_optuna_tuning(
...
if __name__ == '__main__':
    ...
    USE_TOL_FOR_STATS = False # if True, mean/std shading uses only filtered ESNs
    ...
    logging.getLogger("echostate.core.reservoir").setLevel(logging.WARNING)
    logging.getLogger("echostate.core.esn").setLevel(logging.DEBUG)
    logging.getLogger("echostate.core.trainer").setLevel(logging.DEBUG)
            ...
            for col_idx, qubit in enumerate(qubit_list):
                ...
                for i, rseed in enumerate(seeds):
                    ...
                    esn = predictor.make_esn(
                        reservoir_size=best.get("reservoir_size", 800),
                        spectral_radius=best.get("spectral_radius", 0.99),
                        input_scaling=best.get("input_scaling", 0.1),
                        ridge_param=best.get("ridge_param", 0.000001),
                        leak_rate=best.get("leak_rate", 0.2),
                        sparsity=best.get("sparsity", 0.05),
